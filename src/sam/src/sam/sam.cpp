...
namespace sam
{
  ...
  int StreamingAudioManager::jack_process(jack_nframes_t nframes)
  {
    ...
    for (int i = 0; i < m_maxClients; i++)
    {
      if (m_apps[i])
      {
        ...
        {
          qDebug("StreamingAudioManager::jack_process telling app %d to deleteLater, current thread = %p", i, (void*)QThread::currentThreadId());
  ...
  {
    qDebug("StreamingAudioManager::cleanupApp port = %d, type = %d, current thread ID = %p", port, type, (void*)QThread::currentThreadId());
