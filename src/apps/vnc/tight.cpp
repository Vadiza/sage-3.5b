...
VNCViewer::HandleTightBPP (sgVNCViewer *ct, int rx, int ry, int rw, int rh)
{
  ...
  for (stream_id = 0; stream_id < 4; stream_id++) {
    if ((comp_ctl & 1) && zlibStreamActive[stream_id]) {
      if (inflateEnd (&zlibStream[stream_id]) != Z_OK &&
        ...
        SAGE_PRINTLOG("VNC> inflateEnd: %s", zlibStream[stream_id].msg);
  ...
  if (comp_ctl == rfbTightJpeg) {
    SAGE_PRINTLOG("VNC> Tight encoding: JPEG is not supported in 8 bpp mode.");
    return False;
  }
#else
  if (comp_ctl == rfbTightJpeg) {
  ...
  if (comp_ctl > rfbTightMaxSubencoding) {
    SAGE_PRINTLOG("VNC> Tight encoding: bad subencoding value received.");
    return False;
  }
  ...
  if ((comp_ctl & rfbTightExplicitFilter) != 0) {
    ...
    case rfbTightFilterGradient:
      ...
      break;
    default:
      SAGE_PRINTLOG("VNC> Tight encoding: unknown filter code received.");
      return False;
    }
  } else {
    filterFn = &VNCViewer::FilterCopyBPP;
  ...
  if (bitsPixel == 0) {
    SAGE_PRINTLOG("VNC> Tight encoding: error receiving palette.");
    return False;
  }
  ...
  if (compressedLen <= 0) {
    SAGE_PRINTLOG("VNC> Incorrect data received from the server.");
    return False;
  }
  ...
  if (!zlibStreamActive[stream_id]) {
    ...
    if (err != Z_OK) {
      if (zs->msg != NULL)
        SAGE_PRINTLOG("VNC> InflateInit error: %s.", zs->msg);
      return False;
    }
  ...
  if (rowSize > bufferSize) {
    ...
    SAGE_PRINTLOG("VNC> Internal error: incorrect buffer size.");
    return False;
  }
  ...
  while (compressedLen > 0) {
    ...
    do {
      ...
      if (err != Z_OK && err != Z_STREAM_END) {
        if (zs->msg != NULL) {
          SAGE_PRINTLOG("VNC> Inflate error: %s.", zs->msg);
        } else {
          SAGE_PRINTLOG("VNC> Inflate error: %d.", err);
        }
        return False;
      }
  ...
  if (rowsProcessed != rh) {
    SAGE_PRINTLOG("VNC> Incorrect number of scan lines after decompression.");
    return False;
  }
...
VNCViewer::DecompressJpegRectBPP(sgVNCViewer *ct, int x, int y, int w, int h)
{
  ...
  if (compressedData == NULL) {
    SAGE_PRINTLOG("VNC> Memory allocation error.");
    return False;
  }
  ...
  if (cinfo.output_width != w || cinfo.output_height != h ||
    ...
    SAGE_PRINTLOG("VNC> Tight Encoding: Wrong JPEG data received.");
