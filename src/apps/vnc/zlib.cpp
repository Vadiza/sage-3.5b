...
VNCViewer::HandleZlibBPP (sgVNCViewer *ct, int rx, int ry, int rw, int rh)
{
  ...
  if ( decompStreamInited == False ) {
    ...
    if ( inflateResult != Z_OK ) {
      SAGE_PRINTLOG("VNC> inflateInit returned error: %d, msg: %s\n",
  ...
  while (( remaining > 0 ) &&
    ...
    if ( inflateResult == Z_NEED_DICT ) {
      SAGE_PRINTLOG("VNC> zlib inflate needs a dictionary!\n");
      return False;
    }
    if ( inflateResult < 0 ) {
      SAGE_PRINTLOG("VNC> zlib inflate returned error: %d, msg: %s\n",
                    inflateResult, decompStream.msg);
      return False;
    }
    ...
    if (( decompStream.avail_in > 0 ) &&
      ...
      SAGE_PRINTLOG("VNC> zlib inflate ran out of space!\n");
      return False;
    }
