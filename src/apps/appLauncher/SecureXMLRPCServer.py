...
class SecureXMLRPCServer(BaseHTTPServer.HTTPServer,SimpleXMLRPCServer.SimpleXMLRPCDispatcher):
    ...
    def __init__(self, server_address, logRequests=True):
        ...
        self.socket = ssl.wrap_socket(socket.socket(), server_side=True, certfile=CERTFILE,
                                      keyfile=KEYFILE, ssl_version=ssl.PROTOCOL_SSLv23, suppress_ragged_eofs=False)
...
class SecureXMLRPCRequestHandler(SimpleXMLRPCServer.SimpleXMLRPCRequestHandler):
    ...
    def do_POST(self):
        ...
        except Exception as ex: # This should only happen if the module is buggy
            # internal error, report as HTTP server error
            print "Exception occured while handling appLauncher requests: " + ex.message
...
def test():
    ...
    server = SecureXMLRPCServer(LISTEN_HOST, LISTEN_PORT)
