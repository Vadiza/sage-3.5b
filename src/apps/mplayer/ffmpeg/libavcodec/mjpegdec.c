        ...
        for (i = 0; i < nb_components; i++) {
          ...
          for(j=0; j<n; j++) {
            ...
            if(dc == 0xFFFF)
              return -1;
            if(bits<=8){
              ptr = s->picture.data[c] + (linesize * (v * mb_y + y)) + (h * mb_x + x); //FIXME optimize this
              if(y==0 && toprow){
                if(x==0 && leftcol){
                  pred= 1 << (bits - 1);
                }else{
              ...
              *ptr= pred + (dc << point_transform);
              ...
              ptr16 = (uint16_t*)(s->picture.data[c] + 2*(linesize * (v * mb_y + y)) + 2*(h * mb_x + x)); //FIXME optimize this
              if(y==0 && toprow){
                if(x==0 && leftcol){
                  pred= 1 << (bits - 1);
                }else{
