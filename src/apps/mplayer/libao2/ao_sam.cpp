...
static const ao_info_t info ={
  "Sage Audio Manager audio output",
  "sam",
...
static void print_help(void)
{
  mp_msg(MSGT_AO, MSGL_FATAL,
         ...
         "  ip\n"
         "    IP address of the Sage Audio Manager\n"
         "  port\n"
         "    Port for contacting the Sage Audio Manager\n"
         "  type\n"
...
// We use the window title as a name for the SAM connection
//    with SAGE: the filename is passed through the title
...
static int init(int rate, int channels, int format, int flags)
{
  ...
  if (!sam_ip)
  {
    mp_msg(MSGT_AO, MSGL_FATAL, "[SAM] IP address of SageAudioManager not specified: using default 127.0.0.1\n");
  ...
  // First time SAM is used, allocate the SAM object
  if (sac == NULL) {
    sac = new StreamingAudioClient();
  }
